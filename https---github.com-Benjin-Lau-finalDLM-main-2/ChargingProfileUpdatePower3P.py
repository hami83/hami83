...
####################################################################################################################################
def redistribute_charging_capacity(evse_data, line_loads, site_cap=180):
    ...
    # error_correction = 0.01 # to be a variable
    ...
    # line_cap_avail = {line: line_cap - load + C_CP_t_minus_1[line] for line, load in line_loads.items()}                           # Store available line capacity
    ...
    for cp_id, evse in evse_data.items():
        if EVSE_Max_Total_t != 0 and evse['evse_status'] == 0:
            ...
            if evse['phase'] == 'three-phase':  
                ...
                EVSE_proposed_rates[cp_id] = ((EVSE_Max_t[cp_id] * (1 - EV_state_t[cp_id]) / min_EVSE_Max_Phase_t))
                ...
                EVSE_proposed_current[cp_id] =  [min((math.floor(min_line_cap_avail * 10) / 10), \
                                                ...
                                                (math.floor(EVSE_n_max[cp_id] * 10) / 10))]
            ...
            # OTHERWISE:    If EVSE is SINGLE-PHASE, distribute SOLELY across INSTALLED line
                ...
                # CALCULATE:    Proposed EVSE charging rates (%) based on EVSE_Max_t, EV_state_t & EVSE_Max_Total_t
                EVSE_proposed_rates[cp_id] = ((EVSE_Max_t[cp_id] * (1 - EV_state_t[cp_id]) / EVSE_Max_Phase_t[lines])) 
            ...
            print(f"NOTICE: EVSE {cp_id} is not in use or all EVSEs fully charged @ time t")

    ############################################################
    print("Initial Proposed Charging Current: ", EVSE_proposed_current_3P)
    ############################################################
        ...
        for cp_id, evse in EVSE_proposed_current_3P.items():
            ...
            if EVSE_proposed_current_3P[cp_id][0] < EVSE_n_max[cp_id]: 
                if 0 < evse[0] < 6 or evse[0] >= 6:
                                                           # Determine Line Type based off EVSE installation
    ...
    print("*****************************************************")
    print("EVSE Proposed Current (3P): ", EVSE_proposed_current)
...
# # CONVERT: From proposed current value (in three-phase) to proposed power value (in either DC or AC)
# def convert_current_to_power(EVSE_proposed_current_3P, evse_data, site_voltage=400, phase_voltage=240):
#     # CONDITION:    If EVSE is DC, convert current by calculating AC power to DC power
#     if evse_data[cp_id]['evse_type'] == 'DC':
#         EVSEpower_AC = 1.732 * site_voltage * EVSE_proposed_current_3P[cp_id][0]
#         EVSEpower_DC = EVSEpower_AC * evse_data[cp_id]['efficiency']/100
#         EVSE_proposed_power = (math.floor(EVSEpower_DC) * 10)/10
#     # OTHERWISE:    If EVSE is AC, convert current to AC power
#     elif evse_data[cp_id]['evse_type'] == 'AC':
#         if evse_data[cp_id]['phase'] == 'three-phase':
#             EVSEpower_AC = 1.732 * site_voltage * EVSE_proposed_current_3P[cp_id][0]
#         elif evse_data[cp_id]['phase'] == 'single-phase':
#             EVSEpower_AC = phase_voltage * EVSE_proposed_current_3P[cp_id][0]
...
# def update_line_capacity(line_cap_avail, EVSE_proposed_current, site_cap=400):
#     # line_cap = site_cap / 3
#     # line_cap_avail = {line: line_cap - load for line, load in line_loads.items()}    
#     if data['phase'] == 'three-phase':
#         for line in data['line']:
#             line_cap_avail[line] -= EVSE_proposed_current[cp_id][0]
#         # line_cap_avail[data['line'][0]] -= EVSE_proposed_current[cp_id][0]
#         # line_cap_avail[data['line'][1]] -= EVSE_proposed_current[cp_id][0]
#         # line_cap_avail[data['line'][2]] -= EVSE_proposed_current[cp_id][0]
#     elif data['phase'] == 'single-phase':
#         line_cap_avail[data['line']] -= EVSE_proposed_current[cp_id][0]

#     return line_cap_avail

# With reference to cp_id, data in evse_data.items()
# def calculate_proposed_current(line_cap_avail, EVSE_proposed_rates, EVSE_n_max, C_CP_t=100):
#     # GET:          Line type installed/assigned (i.e., L1, L2, L3) for each EVSE
#     lines = data['line']
...
#             # GET:  Numerical value of the line capacity with the least amount of current capacity available               
#             min_line_cap_avail = min(max(line_cap_avail[line], 0) for line in lines)
#             EVSE_proposed_current_3P[cp_id] =   [min((math.floor(min_line_cap_avail * 10) / 10), \
...
#         elif data['phase'] == 'single-phase':
#             correct_line_cap_avail = line_cap_avail[lines] if line_cap_avail[lines] >= 0 else 0
#             EVSE_proposed_current_3P[cp_id] =   [min(correct_line_cap_avail, \
#                                                 (math.floor((EVSE_proposed_rates[cp_id] * C_CP_t) * 10) / 10), \
#                                                 (EVSE_n_max[cp_id]))]
#             EVSE_proposed_current[cp_id] = EVSE_proposed_current_3P[cp_id]
        
#         # UPDATE:   Remove assigned Proposed Current from EACH line
#         update_line_capacity(line_cap_avail, EVSE_proposed_current)
...
#     return EVSE_proposed_current, EVSE_proposed_current_3P
...
#             # CONDITION:    If EVSE is THREE-PHASE, distribute EVENLY across ALL lines
#             calculate_proposed_current(line_cap_avail, EVSE_proposed_rates, EVSE_n_max, C_CP_t)
                
#             # UPDATE:   Add initial proposed current to total allocated current
#             total_allocated_current += EVSE_proposed_current_3P[cp_id][0]

#             # CONVERT: From current to power
#             EVSE_proposed_power[cp_id] = [convert_current_to_power(EVSE_proposed_current_3P, evse_data, site_voltage, phase_voltage)]
...
#                 add_on_current = 0                  # Initialise add-on current to 0
#                 lines = data['line']    # Get line type (i.e., L1 / L2 / L3) for each EVSE
...
# def calculate_proposed_current(evse, line_cap_avail, C_CP_t, EVSE_Max_t, EVSE_Max_Total_t):
#     lines = evse['line'] if isinstance(evse['line'], list) else [evse['line']]
#     min_line_cap_avail = min(line_cap_avail[line] for line in lines)
#     proposed_current_per_phase = min(min_line_cap_avail, (EVSE_Max_t[evse['id']] * (1 - evse['status']) / EVSE_Max_Total_t) * C_CP_t / 3, evse['max_a'] / 3)
#     return proposed_current_per_phase * len(lines)

# def redistribute_charging_capacity(evse_data, line_loads, site_cap=400):
#     C_CP_t = 100
#     EVSE_Max_Total_t = sum(evse['max_a'] * (1 - evse['status']) for evse in evse_data.values())
#     line_cap_avail = {line: site_cap / 3 - load for line, load in line_loads.items()}
    
#     # Initial proposed current calculation
#     for evse in evse_data.values():
#         evse['proposed_current'] = calculate_proposed_current(evse, line_cap_avail, C_CP_t, EVSE_Max_t, EVSE_Max_Total_t)
        
#         # Update line capacities based on proposed current
#         if isinstance(evse['line'], list):
#             for line in evse['line']:
#                 line_cap_avail[line] -= evse['proposed_current'] / len(evse['line'])
#         else:
#             line_cap_avail[evse['line']] -= evse['proposed_current']
    
#     # Calculate power based on the type of charger and efficiency
#     for evse in evse_data.values():
#         if evse['evse_type'] == 'DC':
#             evse['proposed_power'] = evse['proposed_current'] * 400 * math.sqrt(3) * evse['efficiency'] / 100
#         else:  # AC
#             if evse['phase'] == 'three-phase':
#                 evse['proposed_power'] = evse['proposed_current'] * 400 * math.sqrt(3)
#             else:  # Single-phase
#                 evse['proposed_power'] = evse['proposed_current'] * 230
    
#     # Collect proposed power for each EVSE
#     proposed_powers = {evse_id: evse['proposed_power'] for evse_id, evse in evse_data.items()}
#     return proposed_powers
...
evse_data = {
    'LOSa': {'evse_max_a': 16, 'evse_meter': 8, 'evse_status' : 0, 'evse_type': 'AC', 'phase': 'single-phase', 'line': 'L1'}, 
    'LOSb': {'evse_max_a': 16, 'evse_meter': 8, 'evse_status' : 0, 'evse_type': 'AC', 'phase': 'single-phase', 'line': 'L2'},
    ...
    'LOSe': {'evse_max_a': 50, 'evse_meter': 25, 'evse_status' : 0, 'evse_type': 'DC', \
            'efficiency': 94, 'phase': 'three-phase', 'line': ['L1', 'L2', 'L3']},
    'LOSf': {'evse_max_a': 50, 'evse_meter': 25, 'evse_status' : 0, 'evse_type': 'DC',  \
            'efficiency': 94, 'phase': 'three-phase', 'line': ['L1', 'L2', 'L3']},
    'LOSg': {'evse_max_a': 34, 'evse_meter': 12, 'evse_status' : 0, 'evse_type': 'AC', 'phase': 'three-phase', 'line': ['L1', 'L2', 'L3']}
}

# evse_data = {
#     'LOSa': {'evse_max_a': 16, 'evse_meter': 10.2, 'evse_status' : 0, 'evse_type': 'AC', 'phase': 'single-phase', 'line': 'L1'}, 
# }
